...
class DeepFusion(object):
    ...
    def main(split, det_split, is_carla = False):
        data_root = '/home/SamruddhiPai/Desktop/DeepFusionMOT/datasets/kitti/' + split
        detections_name_3D = '3D_pointrcnn_Car_' + det_split
        detections_name_2D = '2D_rrc_Car_' + det_split

        # Define the file path
        calib_root = os.path.join(data_root, 'calib_'+split)
        dataset_dir = os.path.join(data_root,'image_02_'+split)
        ...
        save_root = 'results/'+split   # The root directory where the result is saved
        ...
        detection_file_list_3D = sorted(detection_file_list_3D)
        detection_file_list_2D, num_seq_2D = load_list_from_folder(detections_files_2D, detections_root_2D)
        detection_file_list_2D = sorted(detection_file_list_2D)
        ...
        for seq_file_3D, image_filename in zip(detection_file_list_3D, image_files):
            ...
            ###### change delimiter to , when running with kitti ######
            try:
            ...
            min_frame, max_frame = int(seq_dets_3D[:, 0].min()), len(image_filenames)
            max_frame = int(image_filenames[-1].split('/')[-1].split('.')[0])
            # import pdb; pdb.set_trace()
            print('min and max frames',min_frame, max_frame)
            for frame, img0_path in zip(range(min_frame, max_frame + 1), image_filenames):
                # breakpoint()
                print('frame', frame) 
                print('img_path', img0_path)
                img_0 = cv2.imread(img0_path)
                _, img0_name, _ = fileparts(img0_path)
                # import pdb; pdb.set_trace()
                # temp = seq_dets_3D[:, 0] == frame
                ...
                # import pdb; pdb.set_trace()
                trackers = tracker.update(detection_3D_fusion, detection_2D_only_tlwh, detection_3D_only, detection_3Dto2D_only, additional_info, calib_file_seq)
...
if __name__ == '__main__':
    ...
    split = 'carla-dd3d'
    det_split = 'carla' #'val' if original split is train 
