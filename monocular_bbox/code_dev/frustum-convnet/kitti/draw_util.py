...
def show_image_with_boxes(img, objects, calib, show3d=True, color=(0, 255, 0), scores=[], colors=[], show=True):
    ...
    for i, obj in enumerate(objects):
        ...
        if len(scores) != 0:
            ...
            if area > 25:
                cv2.putText(img1, '%.2f' % scores[i], (int((obj.xmin + obj.xmax) / 2), max(int(obj.ymin) - 3, 0)), cv2.FONT_HERSHEY_PLAIN, fontScale=0.8, color=color, thickness=1, lineType=cv2.LINE_AA)
        # import pdb; pdb.set_trace()
        box3d_pts_2d, box3d_pts_3d = utils.compute_box_3d(obj, calib['P2'])
        # print('box3d_pts_2d',box3d_pts_2d)
        bbox_pred_2d.append(bbo.mbr(box3d_pts_2d))
    ...
    # print('Predited 2D points',bbox_pred_2d)
    return img1, img2, bbox_pred_2d

def show_image_with_boxes3d(img, boxes3d_pts_2d, calib, color=(0, 255, 0), scores=[], colors=[]):
    ''' Show image with 2D bounding boxes '''
    img1 = np.copy(img)  # for 3d bbox
    
    if len(colors) != 0:
        assert len(colors) == len(boxes3d_pts_2d)


    img_width,img_height, _ = img.shape
    # print('Image shape!!!!!!!!', img.shape)
    for i, box3d_pts_2d in enumerate(boxes3d_pts_2d):
        if any(0 > box3d_pts_2d[:,0]) or any(box3d_pts_2d[:,0] >= img_width) or any(0 > box3d_pts_2d[:,1]) or any(box3d_pts_2d[:,1] >= img_height):
            continue
        img1 = draw_projected_box3d(img1, box3d_pts_2d, color)

        if len(scores) != 0:
            xx1, yy1 = np.min(box3d_pts_2d, 0)
            xx2, yy2 = np.max(box3d_pts_2d, 0)
            if (xx2 - xx1) * (yy2 - yy1) > 25:
                cv2.putText(img1, '%.2f' % scores[i], (int((xx1 + xx2) / 2), max(int(yy1) - 3, 0)),
                                cv2.FONT_HERSHEY_PLAIN, fontScale=0.8, color=color, thickness=1, lineType=cv2.LINE_AA)
