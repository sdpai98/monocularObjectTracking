...
def load_velo_scan(velo_filename):
    scan = np.fromfile(velo_filename, dtype=np.float32).reshape((384, 640))
    print('lidar bin file shape before reshape', scan.shape)
    scan = np.resize(scan,(1242,375))
    print('lidar bin file shape before reshape', scan.shape)
    scan = process_depth_map(scan)
    print('lidar bin file shape before reshape', scan.shape)
