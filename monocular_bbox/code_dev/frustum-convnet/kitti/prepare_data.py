...
def extract_frustum_det_data(idx_filename, split, output_filename, det_filename,
    ...
    for data_idx in data_idx_list:
        ...
        # Shape of final depth (384, 640) --> packent
        pc_velo = dataset.get_lidar(data_idx)
        print('Shape of pc_velo, in extract_frustum_det_data',pc_velo.shape)
        # pc_velo = process_depth_map(pc_velo)
        pc_velo[:, 2] = pc_velo[:, 2]*10
        pc_rect = np.zeros_like(pc_velo)
        pc_rect[:, 0:3] = calib.project_image_to_rect(pc_velo[:, 0:3])
        pc_rect[:, 3] = pc_velo[:, 3]
...
def extract_frustum_data(idx_filename, split, output_filename,
    ...
    for data_idx in data_idx_list:
        ...
        # pc_velo = process_depth_map(pc_velo)
        pc_velo[:, 2] = pc_velo[:, 2]*10
        print('PC velo shape after process depth map, in extract_frustum_data')
        pc_rect = np.zeros_like(pc_velo)
        pc_rect[:, 0:3] = calib.project_image_to_rect(pc_velo[:, 0:3])
